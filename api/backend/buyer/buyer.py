...
def create_price_alert():
    ...
    if not all([bookId, userId, targetPrice]):
        return jsonify({'error': 'Missing required fields.'}), 400
    sql = 'INSERT INTO price_alerts (book_id, user_id, target_price) VALUES (%s, %s, %s)'
    try:
        cursor = db.get_db().cursor()
        cursor.execute(sql, [bookId, userId, targetPrice])
        db.get_db().commit()
        cursor.close()
        return jsonify({'message': 'Price alert created.'}), 201
    except Exception as e:
...
def submit_review():
    ...
    if not all([sellerId, buyerId, rating]):
        return jsonify({'error': 'Missing required fields.'}), 400
    sql = 'INSERT INTO reviews (seller_id, buyer_id, rating, comment, created_at) VALUES (%s, %s, %s, %s, %s)'
    try:
        cursor = db.get_db().cursor()
...
def send_message():
    ...
    if not all([senderId, listingId, message]):
        return jsonify({'error': 'Missing required fields.'}), 400
    sql = 'INSERT INTO messages (sender_id, listing_id, message, sent_at) VALUES (%s, %s, %s, %s)'
    try:
        cursor = db.get_db().cursor()
        cursor.execute(sql, [senderId, listingId, message, datetime.now()])
        db.get_db().commit()
        cursor.close()
        return jsonify({'message': 'Message sent.'}), 201
    except Exception as e:
